...
TEST(Constructor, def_const) {
  S21Matrix test_mat;
  EXPECT_EQ(5, test_mat.getRows());
  EXPECT_EQ(5, test_mat.getCols());
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 5; j++) EXPECT_DOUBLE_EQ(0, test_mat(i, j));
}

TEST(Constructor, sec_constr) {
  S21Matrix test_mat(3, 7);
  EXPECT_EQ(3, test_mat.getRows());
  EXPECT_EQ(7, test_mat.getCols());
  for (int i = 0; i < 3; i++)
...
TEST(Constructor, last_constr) {
  ...
  EXPECT_EQ(tmp_mat.getRows(), 0);
  EXPECT_EQ(tmp_mat.getCols(), 0);
  EXPECT_EQ(test_mat.getRows(), 2);
  EXPECT_EQ(test_mat.getCols(), 5);
  for (int i = 0; i < 2; i++)
...
TEST(MulMatrix, incorrect_dim) {
  ...
  try {
    ...
    EXPECT_STREQ("Incorrect input, need cols_1 == rows_2", e.what());
  }
}
...
TEST(Determinant, incorrect_dims) {
  ...
  try {
    ...
    EXPECT_STREQ(e.what(), "Incorrect input, need rows_ == cols_");
  }
}
...
TEST(Inverse, correct_dims) {
  ...
  EXPECT_EQ(inv.getRows(), 3);
  EXPECT_EQ(inv.getCols(), 3);
...
TEST(Inverse, zero_det) {
  ...
  try {
    ...
    EXPECT_STREQ(e.what(), "Incorrect input, need det != 0");
  }
}
...
TEST(plus_operator, correct) {
  ...
  EXPECT_EQ(res_mat.getRows(), 2);
  EXPECT_EQ(res_mat.getCols(), 4);
  for (int i = 0; i < 2; i++)
...
TEST(Index, incorrect_index) {
  ...
  try {
    ...
    EXPECT_STREQ(e.what(), "Indexes outside the matrix");
  }
}
